<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <!-- css -->
    <link rel="stylesheet" href="css/style.css">

    <!-- font -->
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans&display=swap" rel="stylesheet">
    <title>Branding Coffe</title>
</head>
<body>
    <nav class="navbar">
        <img src="images/logo-coffe.png" alt="logo coffe" class="main-logo">
        <ul class="parent-list-nav">
            <li class="list-nav">Home</li>
            <li class="list-nav">Menu</li>
            <li class="list-nav">About Us</li>
            <li class="list-nav">Contact Us</li>
        </ul>
        <div class="autentication">
            <button class="btn btn--login">Login</button>
            <button class="btn btn--register">Register</button>
        </div>
    </nav>
    <main class="main-section">
        <section class="section-one">
            <div class="container-title">
                <h1 class="main-title">Take your time and <span>stay</span> connected with <span>connections</span></h1>
                <p class="title-description">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Mauris hendrerit massa id blandit pretium. Mauris semper nunc ut sapien.</p>
            </div>
        </section>
        <section class="section-two">
            <div class="container-first-part">
                <img src="images/section-one--image-first-part.png" alt="Coffe Grinder" class="image-first-part">
            </div>
            <div class="container-second-part">
                <img src="images/section-two--image-second-part.png" alt="Coffe Spoon" class="image-second-part">
                <div class="text-second-part">
                    <div class="text-top-part">
                        <h3 class="top-part-tertiary-text">How we <span>give</span> the best <span>for you</span></h3>
                        <p class="top-part-description-text">Quisque faucibus est odio, ac egestas ex aliquam nec. Donec et imperdiet sapien. Suspendisse egestas, augue in egestas bibendum, lorem justo volutpat tellus, at pellentesque tortor ante a felis.</p>
                    </div>
                    <div class="text-bottom-part">
                        <div class="text-bottom-first">
                            <div class="quality-box">
                                <img src="images/absolute-rectangle.png" alt="Absolute Rectangle" class="rectangle">
                                <h4 class="quality">Quality</h4>
                            </div>
                            <p class="quality-description">many businesses trust reals as their warehouse manager</p>
                        </div>
                        <div class="text-bottom-second">
                            <div class="experient-box">
                                <img src="images/absolute-rectangle.png" alt="Absolute Rectangle" class="rectangle">
                                <h4 class="experient">Experient</h4>
                            </div>
                            <p class="experient-description">35 years of experience in the field</p>
                        </div>
                    </div>
                </div>
            </div>
        </section>
        <section class="section-three">
            <img src="images/logo-center.png" alt="Center Logo" class="center-logo">
        </section>
        <section class="section-four">
            <div class="section-four-text">
                <h2 class="section-four-secondary-text">Our <span>signature</span> menu and <span>best</span> seller</h2>
                <p class="section-four-description-text">Lorem ipsum is a name for a common type of placeholder text. Also known as filler or dummy text</p>
            </div>
            <div class="section-four-image">
                <img src="images/section-four--main-image.png" alt="Coffe Variant" class="section-four-main-image">
                </div>
        </section>
        <section class="section-five">
            <div class="section-five-text">
                <h2 class="section-five-secondary-text">Our comfort <span>place</span></h2>
                <p class="section-five-description-text">Don’t take our word for it — hear what our customers are saying</p>
            </div>
            <div class="section-five-image">
                <img src="images/section-five--main-image.png" alt="Coffe Place" class="section-five-main-image">
                </div>
        </section>
    </main>
</body>
</html>